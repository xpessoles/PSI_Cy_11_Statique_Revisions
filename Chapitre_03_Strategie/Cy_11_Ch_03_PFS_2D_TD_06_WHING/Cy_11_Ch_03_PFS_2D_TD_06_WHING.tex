%"{'classe':('PSI'),'chapitre':'stat_pfs_2d','type':('td'),'titre':'WHING : WHeelchair Initiative New Generation', 'source':'Concours CCP TSI 2017','comp':('B2-14','C1-05','C2-07','STAT-02','STAT-04'),'corrige':True}"
%\setchapterimage{fig_02}
\chapter*{TD \arabic{cptTD} :
WHING : WHeelchair Initiative New Generation \ifnormal $\star$ \else \fi \ifdifficile $\star\star$ \else \fi \iftdifficile $\star\star\star$ \else \fi
...
\marginnote{Concours CCP TSI 2017.}
\marginnote{\xpComp{STAT}{02}\xpComp{STAT}{04}}
...
\item le couple $\vect{\indice{C}{red}}$, couple à la sortie du réducteur du moteur-roue.
\end{itemize}}
\ifprof
\begin{corrige}
\end{corrige}
...
\question{Isoler la roue motrice et déterminer une équation issue du PFS donnant $\vect{\indice{C}{red}}$ en fonction des données géométriques, de $\vect{N_{BS}}$ et $\vect{T_{BS}}$.}
\ifprof
\begin{corrige}
\end{corrige}
\else
